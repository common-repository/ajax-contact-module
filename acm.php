...
Plugin URI: http://www.danycode.com/ajax-contact-module/
...
function acm_options() {
    ...
    if(isset($_POST[label1]) or isset($_POST[label2]) or isset($_POST[label3]) or isset($_POST[label4])){        
        ...
        if($error==0){
			...
			echo '<p class="acm-saved">Your options have been saved</p>';
		}else{
			//show error message
	...
	echo '<p>Ask for support at <a target="_blank" href="http://www.danycode.com/ajax-contact-module/">Ajax Contact Module Official Page</a></p>';
	echo '</form>';
	echo '</div>';   	
